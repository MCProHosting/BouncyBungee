...
public class TCommandDispatch {
    ...
    public void onCommand(CommandSender sender, TCommand meta, String[] strings) {
        try {
            ...
            if (!validType) {handler.handleCommandStatus(TCommandStatus.WRONG_TARGET, sender, typesender); return;}
            if (!sender.hasPermission(meta.permission())) {handler.handleCommandStatus(TCommandStatus.PERMISSIONS, sender, typesender); return;}
            Object invoke = method.invoke(handler, sender, typesender, meta, strings);
            if (!(invoke instanceof TCommandStatus)) throw new TCommandException("Invalid value returned for command status!");
